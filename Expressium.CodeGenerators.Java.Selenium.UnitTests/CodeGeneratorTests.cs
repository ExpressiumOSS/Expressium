...
namespace Expressium.CodeGenerators.Java.Selenium.UnitTests
{
    ...
    public class CodeGeneratorTests
    {
        ...
        public void CodeGenerator_GenerateAll()
        {
            ...
            var mainSourcePath = Path.Combine(directory, "src", "main", "java", packagePath);
            var testSourcePath = Path.Combine(directory, "src", "test", "java", packagePath);

            var loginPageFile = Path.Combine(mainSourcePath, "pages", "LoginPage.java");
            if (File.Exists(loginPageFile))
                File.Delete(loginPageFile);

            var loginModelFile = Path.Combine(mainSourcePath, "models", "LoginPageModel.java");
            if (File.Exists(loginModelFile))
                File.Delete(loginModelFile);

            var loginTestFile = Path.Combine(testSourcePath, "uitests", "LoginPageTests.java");
            if (File.Exists(loginTestFile))
                File.Delete(loginTestFile);

            var loginFactoryFile = Path.Combine(testSourcePath, "factories", "LoginPageModelFactory.java");
        ...
        public void CodeGenerator_GeneratePage()
        {
            ...
            var loginPageModelFile = Path.Combine(mainSourcePath, "models", "LoginPageModel.java");
