...
namespace Expressium.CodeGenerators.CSharp.Playwright
{
    internal class CodeGeneratorTest : CodeGeneratorObject
    {
        ...
        internal List<string> GenerateOneTimeSetUpMethod(ObjectRepositoryPage page)
        {
            ...
            listOfLines.Add($"public async Task OneTimeSetUp()");
            listOfLines.Add($"{{");
            ...
            if (!string.IsNullOrWhiteSpace(configuration.CodeGenerator.InitialLoginPage) && !configuration.CodeGenerator.InitialLoginPage.Contains(page.Name))
                listOfLines.Add($"await InitializeBrowserWithLogin();");
            else
                listOfLines.Add($"await InitializeBrowser();");
            listOfLines.Add($"");
            ...
            listOfLines.Add($"{page.Name.CamelCase()} = new {page.Name}(logger, page);");

            if (page.Model)
            {
                listOfLines.Add($"await {page.Name.CamelCase()}.FillForm({page.Name.CamelCase()}Model);");
            }
            else
            {
                foreach (var control in page.Controls)
                {
                    if (control.IsFillFormControl())
                        listOfLines.Add($"await {page.Name.CamelCase()}.Set{control.Name}({control.Name.CamelCase()});");
                }
            }
            ...
            foreach (var page in objectRepository.Pages)
            {
                ...
                foreach (var control in page.Controls)
                {
                    if (control.Target == name)
                    {
                        ...
                        listOfLines.Add($"var {page.Name.CamelCase()} = new {page.Name}(logger, page);");
                        listOfLines.Add($"await {page.Name.CamelCase()}.Click{control.Name}();");
                        listOfLines.Add($"");
        ...
        internal List<string> GenerateFillFormTestMethods(ObjectRepositoryPage page)
        {
            ...
            foreach (var control in page.Controls)
            {
                if (control.IsFillFormControl())
                {
                    ...
                    listOfLines.Add($"public async Task Validate_Page_Property_{control.Name}()");
                    listOfLines.Add($"{{");
