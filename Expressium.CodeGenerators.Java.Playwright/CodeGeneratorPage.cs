...
namespace Expressium.CodeGenerators.Java.Playwright
{
    internal class CodeGeneratorPage : CodeGeneratorObject
    {
        ...
        internal List<string> GenerateImports(ObjectRepositoryPage page)
        {
            ...
            listOfLines.Add("import com.microsoft.playwright.*;");
        ...
        internal List<string> GenerateContructor(ObjectRepositoryPage page)
        {
            ...
            listOfLines.Add($"public {page.Name}(Logger logger, Page page) {{");
            listOfLines.Add($"super(logger, page);");

            foreach (var member in page.Members)
                listOfLines.Add($"this.{member.Name.CamelCase()} = new {member.Page}(logger, page);");

            foreach (var control in page.Controls)
            {
                if (control.IsTable())
                    listOfLines.Add($"this.{control.Name.CamelCase()} = new BaseTable(logger, page, {GetPlaywrightLocator(control)});");
            }
            ...
            foreach (var control in page.Controls)
            {
                if (!control.IsTable())
                {
                    ...
                    if (type == ControlTypes.Element.ToString())
                        type = "Control";
                    listOfLines.Add($"this.{control.Name.CamelCase()} = new Web{type}(page, {GetPlaywrightLocator(control)});");
                }
            }
        ...
        internal string GetPlaywrightLocator(ObjectRepositoryControl control)
        {
            switch (control.How)
            {
                case "Id": return $"page.locator(\"[id='{control.Using}']\")";
                case "Name": return $"page.locator(\"[name='{control.Using}']\")";
                case "ClassName": return $"page.locator(\".{control.Using}\")";
                case "CssSelector": return $"page.locator(\"{control.Using.EscapeDoubleQuotes()}\")";
                case "XPath": return $"page.locator(\"{control.Using.EscapeDoubleQuotes()}\")";
                case "Label":
                    var role = GetAriaRole(control.Type);
                    return $"page.getByRole(AriaRole.{role}, new Page.GetByRoleOptions().setName(\"{control.Using.EscapeDoubleQuotes()}\").setExact(true))";
                default: return $"page.locator(\"{control.Using.EscapeDoubleQuotes()}\")";
            }
        }

        internal string GetAriaRole(string controlType)
        {
            switch (controlType)
            {
