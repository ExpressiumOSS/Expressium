...
namespace Expressium.CodeGenerators.Java.Playwright
{
    internal class CodeGeneratorObject
    {
        ...
        {
            return $"{configuration.Company}.{configuration.Project}.web.api".ToLower();
        }
        ...
        internal string GetMainSourcePath()
        {
            return Path.Combine(configuration.SolutionPath, "src", "main", "java", GetPackagePath());
        }

        internal string GetTestSourcePath()
        {
            return Path.Combine(configuration.SolutionPath, "src", "test", "java", GetPackagePath());
        }
    }
}
