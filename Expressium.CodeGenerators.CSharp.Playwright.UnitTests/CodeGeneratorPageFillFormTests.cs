...
namespace Expressium.CodeGenerators.CSharp.Playwright.UnitTests
{
    ...
    public class CodeGeneratorPageFillFormTests
    {
        ...
        public void CodeGeneratorPageCSharp_GenerateFillFormMethod()
        {
            ...
            Assert.That(listOfLines[0], Is.EqualTo("public async Task FillForm(FillFormPageModel model)"), "CodeGeneratorPageCSharp GenerateFillFormMethod validation");
            ...
            Assert.That(listOfLines[3], Is.EqualTo("await SetGender(model.Gender);"), "CodeGeneratorPageCSharp GenerateFillFormMethod validation");
            Assert.That(listOfLines[4], Is.EqualTo("await SetTransport(model.Transport);"), "CodeGeneratorPageCSharp GenerateFillFormMethod validation");
            Assert.That(listOfLines[5], Is.EqualTo("await SetAgreement(model.Agreement);"), "CodeGeneratorPageCSharp GenerateFillFormMethod validation");
            Assert.That(listOfLines[6], Is.EqualTo("await SetSection(model.Section);"), "CodeGeneratorPageCSharp GenerateFillFormMethod validation");
        }
